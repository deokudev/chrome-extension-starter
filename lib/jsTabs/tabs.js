...
Tabs.prototype = {
  fireOnToggle_: function (tab_name) {
    if (
      this.ontoggle_handlers_list.hasOwnProperty(tab_name) &&
      this.ontoggle_handlers_list[tab_name].length > 0
    ) {
  ...
  toggle: function (tab_name) {
    if (tab_name && this.nav_links_array.indexOf(tab_name) !== -1) {
      [].forEach.call(
        this.html.querySelectorAll(".tabs_content>div"),
        function (el) {
  ...
  onNavClick_: function () {
    let self = this;
    self.html.querySelector(".tabs_nav").addEventListener(
      "click",
      function (e) {
        ...
        if (
          hash &&
          self.nav_links_array.indexOf(hash) !== -1 &&
